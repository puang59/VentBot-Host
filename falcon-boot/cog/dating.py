...
class dating(commands.Cog):
    ...
    class cycleButton(discord.ui.View):
        ...
        async def done_button(self, interaction:discord.Interaction, button:discord.ui.Button):
            ...
            if dUser.find_one({"user": interaction.user.id}):
                intuser = dUser.find_one({"user": interaction.user.id})
                mem = interaction.guild.get_member(intuser['user'])
                chn = interaction.guild.get_channel(intuser['channel'])
                await chn.set_permissions(mem, send_messages=True, view_channel=True)
                

            # Age checker 
            if int(results['age']) < 13:
                try:
                    await interaction.user.send("Sorry! You are little too young for both discord and dating")
                except:
                    pass
    ...
    async def on_member_join(self, member):
        ...
        if member.guild.id == 999682901308342342:
            if collection.find_one({"author_id": member.id}):
                ...
                categories = ["PRIVATE SPACE", "PRIVATE SPACE (2)", "PRIVATE SPACE (3)","PRIVATE SPACE (4)","PRIVATE SPACE (5)",\
                ...
                for categName in categories:
                    try: 
                        ...
                        await text_channel.set_permissions(user_a, send_messages=False, view_channel=True)
                        ...
                        await text_channel.edit(slowmode_delay=5)

                        postUser = {"user": member.id, "channel": text_channel.id}
                        dUser.insert_one(postUser)
                        ...
                        if collection.find_one({"_id": member.id}):
                            ...
                            try:
                                msg2 = await self.bot.wait_for("message", check=check_join, timeout=600)
                                ...
                                return
                            if msg2: 
                                await thre.send('What is your sexuality? [Straight, Gay, Lesbian, Bisexual, Demidesexual etc]')

                                try:
                                    msg1 = await self.bot.wait_for("message", check=check_join, timeout=600)
                                except asyncio.TimeoutError:
                                ...
                                if msg1:
                                    ...
                                    try:
                                        msg3 = await self.bot.wait_for("message", check=check_join, timeout=600)
                                    except asyncio.TimeoutError:
                                        error5 = await thre.send("<:disagree:943603027854626816> Introduction process failed")
                                        await asyncio.sleep(5)
                                        await error5.delete()
                                    ...
                                    if msg3:
                                        ...
                                        try:
                                            msg6 = await self.bot.wait_for("message", check=check_join, timeout=600)
                                        except asyncio.TimeoutError:
    ...
    async def on_member_remove(self, member):
        if member.guild.id == 999682901308342342:
            ...
            if collection.find_one({"_id": member.id}):
                ...
                await txt.delete()
            dUser.delete_many({"user": member.id})
            gChat.delete_many({"_id": member.id})
    ...
    async def intro(self, ctx):
        ...
        if not isinstance(ctx.message.channel, discord.Thread):
            ...
            if collection.find_one({"_id": ctx.author.id}):
                p = {"mem": ctx.author.id, "inProcess": "true"}
                dUser.insert_one(p)
                ...
                if msg.content.lower() == "y" or msg.content.lower() == "yes":
                    ...
                    dUser.delete_many({'mem': ctx.author.id})
                    return 
